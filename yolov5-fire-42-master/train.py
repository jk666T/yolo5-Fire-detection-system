...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    # 检查是否有标签数据
    if dataset.labels and all(len(l) > 0 for l in dataset.labels):
    ...
    if RANK in [-1, 0]:
        ...
        if not resume:
            ...
            # Anchors
            if not opt.noautoanchor and dataset.labels and any(len(l) > 0 for l in dataset.labels):
                check_anchors(dataset, model=model, thr=hyp['anchor_t'], imgsz=imgsz)
            else:
                LOGGER.warning('Skipping anchor check due to lack of labels')
...
def parse_opt(known=False):
    ...
    parser.add_argument('--cfg', type=str, default='mask_yolov5s.yaml', help='model.yaml path')
    parser.add_argument('--data', type=str, default='fire_data.yaml', help='dataset.yaml path')
